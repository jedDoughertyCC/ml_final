...
names(plotter) <- c("movie","break_even","big_money","earnings_ratio","budget_raw",
                    "worldwide_gross","production_budget")

plotter$even <- "Double Production Budget"

plotter[plotter$big_money == -1,]$even <- "Lost Money"




movie_plot <- ggplot(plotter,aes(budget_raw,earnings_ratio)) +
              geom_point(aes(colour = even), alpha = .7) +
              scale_y_log10()
